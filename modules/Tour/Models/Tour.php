...
//     }
    ...
    //     $tour_date = TourDate::distinct()->where('active',1)->whereRaw('substr(start_date,1,10) >= ?',[$check_date])->get(['target_id']);
        
    //     foreach($tour_date as &$row){
    //         $model_Tour = parent::query()->select("bravo_tours.*");
    //         $model_Tour->where("bravo_tours.id", $row->target_id);
    //         $model_Tour->where("bravo_tours.status", "publish");
            
    //         if (!empty($location_id = $request->query('location_id'))) {
    //             if ($location_id!='0') {
    //                $model_Tour->where('location_id',$location_id);
    //             }
    //         }
    ...
    //         $temp_tour = $model_Tour->with(['location:id,name','hasWishList'])->get();
    //         // dd($row->variants);
            
    //         foreach($temp_tour as &$rv){
    //             $rv->price=0;
    //             $rv->availability = TourDate::where('target_id',$rv->id)->where('active',1)->whereRaw('substr(start_date,1,10) >= ?',[$check_date])->orderBy('start_date','asc')->get();
    //             if($rv->image_id){
    //                 $media = MediaFile::find($rv->image_id);
    //                 $pre_url = 'https://cdn.mykomodo.kabtour.com/uploads/';
    //                 $rv->banner = [
    ...
    //                 $rv->gallery = $arrGall;
    //             }
    //             foreach($rv->availability as $rva){
    //                 if($rv->price==0){
    //                     $rv->price = $rva->price;
    //                 }else{
    //                     if($rv->price<$rva->price){
    //                         $rv->price=$rva->price;
    //                     }
    //                 }
    //             }
            ...
            foreach($temp_tour as &$rv){
                ...
                foreach($rv->availability as $rva){
                    if($rv->price==0){
                        ...
                        if(substr($rva->start_date,0,10)==$check_date){
                            $rv->price = $rva->price;
                        }
                    }
                }
    ...
    public static function myProducts(Request $request)
    {
        ...
        $list = $model_Tour->with(['location:id,name','hasWishList'])->get();
        ...
        foreach($list as &$rv){
            ...
            // $rv->price = (string)0;
            $rv->price_normal = $rv->price;
            $rv->special_dates = TourDate::where('target_id',$rv->id)->get();
            $checkCal = TourDate::where('target_id',$rv->id)->where('price_date',$check_date)->where('active',1)->first();
            if($checkCal){
                $rv->price = $rv->price_holiday;
            }
        }
        return $list;
    }
